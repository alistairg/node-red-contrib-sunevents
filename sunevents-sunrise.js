...
module.exports = function(RED) {
	...
	function SunriseNode(config) {
    	...
    	node.modes = {test: config.testmode, debug: config.verbose, filters: ["sunrise"], offset: config.offset}
    	...
    	node.events.on("sunevent", function(event, date) {
    		var msg = {}
    		msg.topic = node.topic || node.name || 'sunrise'
    		msg.payload = event
    		msg.datetime = date
	...
	RED.nodes.registerType("sun-sunrise", SunriseNode);
