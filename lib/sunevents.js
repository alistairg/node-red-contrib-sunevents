...
function SunEvents(lat, lng, modes) {
	...
	this.filters = modes.filters;
	this.offset = modes.offset || 0;
	...
	function setTimers(date) {
		...
		Object.keys(times).forEach(function(event) {
			...
			if ((!self.filters) || ((self.filters) && (self.filters.indexOf(event) != -1))) {

				var eventMoment = moment(times[event]).add(self.offset, 'minutes');
				...
				debug("Ignoring event %s - filtered by mode filter", event);
			}
